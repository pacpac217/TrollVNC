...
static void initializeAndRunRfbServer(void) {
    ...
    if (isRepeaterEnabled()) {
        ...
        if (gRepeaterMode == 2) {
            ...
            // Send device ID immediately after connection (before VNC handshake)
            // This allows server to identify which device is connecting
            if (sClient && sClient->sock >= 0) {
                // Pad device ID to 32 bytes for consistent parsing on server
                NSMutableString *paddedDeviceId = [NSMutableString stringWithString:deviceId];
                while (paddedDeviceId.length < 32) {
                    [paddedDeviceId appendString:@"\0"];
                }
                if (paddedDeviceId.length > 32) {
                    paddedDeviceId = [[paddedDeviceId substringToIndex:32] mutableCopy];
                }
        ...
        if (!sClient) {
            ...
            if (sRetryInterval > 0) {
                TVLog(@"Waiting %.1f seconds before retry...", sRetryInterval);
                CFRunLoopRunInMode(kCFRunLoopDefaultMode, sRetryInterval, false);
                // Retry connection instead of exiting
                TVLog(@"Retrying reverse connection...");
                // Will be retried by watchdog or restart logic
            }
            exit(EXIT_FAILURE);
        }
        ...
        // Note: clientGoneHook is already set in newClientHook
        // When reverse connection is lost (e.g., due to IP change), 
        // clientGoneHook will be called, which stops the run loop.
        // The watchdog (trollvncmanager) will then restart the process,
        // which will automatically reconnect with the new IP.
